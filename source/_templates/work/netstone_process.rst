...
2. 以下流程中，除8以外，必须按顺序执行，第8条特别耗费时间，建议先执行；
         
   
具体流程
-------------------------------------------------
1. 将线上石头网的member表和authorise表复制到大平台中，将member表改为member_net别名；
...
7. 将石头网比大平台多的用户导入到大平台中，缺少的字段置默认值；
   ...
   qq_number, email) SELECT username, PASSWORD, enabled, account_non_expired, CREDENTIALS_NON_EXPIRED, reg_time, realname, phone, qq, 
   email FROM member_net WHERE NOT EXISTS (SELECT * FROM member WHERE member.username = member_net.username)

8. 将大平台比石头网多的用户导入到石头网的临时表中，缺少的字段置默认值；
   
   INSERT INTO member_net(username, PASSWORD, enabled, account_non_expired, CREDENTIALS_NON_EXPIRED, reg_time, realname, phone, qq, email) 
   ...
   FROM member WHERE NOT EXISTS (SELECT * FROM member_net WHERE member_net.username=member.username);
